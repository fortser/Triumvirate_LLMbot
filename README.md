...
Пять компонентов для последовательного анализа и оптимизации качества игры LLM-моделей:

```
python -m trace_analyzer.metrics [--smartbot]  →  /evaluate-models  →  /analyze-strategy  →  /optimize-prompts  →  /evaluate-optimization
       (автометрики + SmartBot)                    (рейтинг моделей)    (шахматный анализ)     (промпт-рекомендации)    (Go/No-Go решения)
```

| Компонент | Тип | Команда | Назначение |
|-----------|-----|---------|-----------|
| **metrics.py** | Python CLI | `python -m trace_analyzer.metrics` | Автоматические метрики, рейтинг моделей |
| **Model Evaluator** | Claude Code agent | `/evaluate-models` | Шахматная оценка ходов, пригодность моделей |
| **Chess Strategy Analyst** | Claude Code agent | `/analyze-strategy` | Глубокий анализ стратегии и тактики по 7 аспектам |
| **Prompt Optimizer** | Claude Code agent | `/optimize-prompts` | Анализ промптов, рекомендации по оптимизации |
| **Docs Keeper** | Claude Code agent | `/update-docs` | Актуализация CHANGELOG, оглавления и README |
| **Optimization Evaluator** | Claude Code agent | `/evaluate-optimization` | Оценка эффекта промпт-оптимизаций, Go/No-Go решения |

### Chess Strategy Analyst — глубокий шахматный анализ

Агент `/analyze-strategy` анализирует трейсы ходов по 7 аспектам с рубриками 0-5 (макс. 35 баллов):

| # | Аспект | Что оценивается |
|---|--------|-----------------|
| 1 | **Стратегия** | Оценка силы/слабости оппонентов, баланс агрессии между двумя противниками |
| 2 | **Тактика** | Чтение позиции, расчёт вариантов, количество кандидатов |
| 3 | **Угрозы** | Распознавание входящих угроз, реакция на шахи и атаки |
| 4 | **Атаки** | Поиск захватов, шахов, вилок, тактических мотивов |
| 5 | **Дебют** | Развитие фигур, контроль центра, план на первые 8 ходов |
| 6 | **Эндшпиль** | Реализация преимущества, выживание, промоушен пешек |
| 7 | **Адаптация** | Смена стиля в выигрышных/проигрышных позициях |

Режимы работы:
- `/analyze-strategy` — полный отчёт по топ-6 моделям
- `/analyze-strategy deepseek/deepseek-v3.2` — deep dive одной модели
- `/analyze-strategy дебют` — фокус на одном аспекте по всем моделям
- `/analyze-strategy game_<uuid>` — ход-за-ходом разбор конкретной игры
...
│       └── optimization_log.json          # Лог фаз оптимизации (baseline, results, rollbacks)
│
...
│   ├── agents/                            # 5 агентов: model-evaluator, chess-strategy-analyst, prompt-optimizer, docs-keeper, optimization-evaluator
│   ├── skills/                            # 7 навыков: chess-strategy-analyst + тестирование + документация + optimization-evaluator
