...
Четыре компонента для последовательного анализа качества игры LLM-моделей:

```
python -m trace_analyzer.metrics [--smartbot]  →  /evaluate-models  →  /analyze-strategy  →  /optimize-prompts
       (автометрики + SmartBot)                    (рейтинг моделей)    (шахматный анализ)     (промпт-рекомендации)
```
...
│   ├── agents/                            # 4 агента: model-evaluator, chess-strategy-analyst, prompt-optimizer, docs-keeper
│   ├── skills/                            # 6 навыков: chess-strategy-analyst + тестирование + документация
