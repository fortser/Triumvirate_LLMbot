...
> **Дата анализа:** 2026-03-20 (обновлено 2026-03-22)
> **Данные:** 5195 ходов, 22 модели, 43 игры
> **SmartBot evaluation:** 5173 хода оценены объективно
> **Win rate всех моделей:** 0%
>
> **ВАЖНО (2026-03-22):** Метрика `threat_addressed_rate` была сломана из-за опечатки
> в `smartbot_adapter.py:315` (`critical_count` вместо `critical_threats`).
> После исправления реальные значения: 0.85-0.97 (модели адресуют угрозы в 85-97% случаев).
> Проблема P-001 ниже основана на ошибочных данных — модели НЕ слепы к угрозам.

---

## Часть I. Выявленные проблемы и недостатки

### ~~P-001. Модели не адресуют угрозы~~ (ОПРОВЕРГНУТО 2026-03-22)

- **Было:** `smartbot_threat_addressed_rate = 0.0` у ВСЕХ 22 моделей
- **Причина:** баг в `smartbot_adapter.py:315` — обращение к несуществующему атрибуту `critical_count` вместо `critical_threats`. Метрика не вычислялась, а не модели не адресовали угрозы
- **Реальность после фикса:** threat_addressed_rate = 0.85-0.97. Модели адресуют угрозы в 85-97% случаев
- **Статус:** НЕ является проблемой промптов. Промпт-оптимизация для угроз — LOW priority
...
- ~~Повышение threat_addressed_rate с 0.0 до >0.3~~ (уже 0.85-0.97 после фикса бага)
...
- `smartbot_threat_addressed_rate` (уже 0.85-0.97 после фикса бага; цель: поддерживать >0.85)
